...
- **`podman generate systemd` is deprecated**: This command is deprecated upstream in favour of Quadlet and will receive only urgent bug fixes. It also hardcodes revision-specific snap paths (e.g. `/snap/m0x41-podman/x1/usr/bin/podman`) that break on snap refresh. Use Quadlet `.container` files instead.
...
| CentOS 9 | 7/7 | 8/8 | 6/6 | 15/16 |
| Fedora 42 | 5/7 | 1/8 | 6/6 | 14/16 |

CentOS 9 and Fedora 42 tier 5 failures are rootless Quadlet tests — same underlying `newuidmap` setuid / `dbus-user-session` limitation in LXD containers that affects tier 2. Rootful Quadlet passes on all distros.
