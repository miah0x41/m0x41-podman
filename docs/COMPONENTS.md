...
|-------|-------|--------|-----|
| `healthcheck-ld-library-path.patch` | `libpod/healthcheck_linux.go` | Override binary path via `PODMAN_BINARY`, propagate `LD_LIBRARY_PATH` and `CONTAINERS_*` config env vars via `--setenv` when creating transient systemd healthcheck units | _Podman_ embeds its own binary path (via `/proc/self/exe`) in transient systemd units for container healthchecks. In the snap, this resolves to the raw binary inside the snap filesystem, bypassing the shim's environment setup. The patch makes three changes: (1) checks `PODMAN_BINARY` and overrides the resolved binary path so the transient unit's `ExecStart` references the shim at `/usr/local/bin/podman`; (2) propagates `LD_LIBRARY_PATH` via `systemd-run --setenv`, mirroring the existing `PATH` propagation; (3) propagates `CONTAINERS_CONF`, `CONTAINERS_REGISTRIES_CONF`, and `CONTAINERS_STORAGE_CONF` so the transient unit uses the snap's bundled config. Without (1) and (3), healthcheck timers fail with `could not find "netavark"`; without (2), they fail with `libgpgme.so.11: cannot open shared object file`. |
...
The healthcheck patch follows the existing upstream code pattern (identical to how `PATH` is already propagated). The `LD_LIBRARY_PATH` and `CONTAINERS_*` propagations have no effect when the respective variables are unset. The `PODMAN_BINARY` override falls back to `os.Executable()` when unset — the same pattern used in the binary path patch below. See [HEALTHCHECK_ISSUES.md](investigations/HEALTHCHECK_ISSUES.md) for the full root cause analysis and [PATCH_SECURITY_REVIEW.md](investigations/PATCH_SECURITY_REVIEW.md) for the security review.
